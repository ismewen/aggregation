...
class HMTBaseException(Exception):
    ...
    def api_response(self):
        return api_response(self.json_response(), status=self.status)


class HMTGeneralAPIError(HMTBaseException):
    code = 1000


class HMTGeneralAuthenticateError(HMTGeneralAPIError):
    code = 2000
